...
public void onPlayerHitAnchor(EntityDamageByEntityEvent e)
{
  ...
  if (Habilidade.getAbility(p).equalsIgnoreCase("Anchor"))
  {
    ...
    NewKitMenu.playSound(a, Main.getInstace().getConfig().getString("Sound.AnchorHit"), 1.0F, 1.0F);

    for (Entity ent : p.getNearbyEntities(12, 12, 12)) {
    	if (ent instanceof Player) {
    	      NewKitMenu.playSound(((Player) ent), Main.getInstace().getConfig().getString("Sound.AnchorHit"), 1.0F, 1.0F);
...
/*  72 */        NewKitMenu.playSound(p, Main.getInstace().getConfig().getString("Sound.AnchorHit"), 1.0F, 1.0F);
return true;
/*     */       }
...
/* 36 */        NewKitMenu.playSound(p, Main.getInstace().getConfig().getString("Sound.AnchorHit"), 1.0F, 1.0F);
return true;
/*    */       }
